# app/countdown.py
"""
Beregning av target + tick. Håndterer minus-tid i daglig modus.
"""
...
# Monotontid: stabil mot NTP-hopp
...
def _next_daily_target_with_overrun(hhmm: str, now_dt: datetime, overrun_ms: int) -> int:
    # Hvorfor: la minus-tid vises for dagens target før vi ruller til i morgen
    t = _parse_hhmm(hhmm)
    today = now_dt.replace(hour=t.hour, minute=t.minute, second=0, microsecond=0)
    if now_dt <= today:
        return _ms_from_dt(today)
    # nå er vi etter dagens target
    if (now_dt - today) <= timedelta(milliseconds=overrun_ms):
        return _ms_from_dt(today)  # behold dagens for minus-tid
    # etter overrun -> i morgen
    tomorrow = today + timedelta(days=1)
    return _ms_from_dt(tomorrow)
...
def compute_tick(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if target_ms <= 0:
        ...
        if signed > 0:
            ...
            if -signed <= overrun_ms:
                ...
                display_ms = max(0, overrun_ms + signed)  # signed<0
            else:
                state = "ended"; phase = "ended"; blink = False; display_ms = 0

    try:
        target_hhmm = datetime.fromtimestamp(target_ms / 1000, tz=TZ).strftime("%H:%M") if target_ms > 0 else ""
    except Exception:
        target_hhmm = ""
    ...
    return {
        ...
        "state": state,          # countdown|overrun|ended|idle
        "mode": phase,           # normal|warn|alert|over|ended
        "blink": bool(blink),
        "warn_ms": warn_ms,
        "alert_ms": alert_ms,
        "overrun_ms": int(cfg.get("overrun_minutes", 1)) * 60_000,
