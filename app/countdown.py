...
def _next_daily_target_with_overrun(
    ...
    """Finn neste daglige mål, med overrun-vindu."""
    t = _parse_hhmm(hhmm)
...
def compute_target_ms(cfg: Dict[str, Any], *, now_ms: Optional[int] = None) -> int:
    """Beregner neste target i ms basert på config."""
...
def compute_tick(cfg: Dict[str, Any]) -> Tick:
    """Hoved-funksjon for å generere status/tick for frontend."""
    now_ms = _now_ms()
    mode_cfg = cfg.get("mode", "daily")

    # clock-modus → ingen target
    if mode_cfg == "clock":
        return Tick(
            now_ms=now_ms,
            target_ms=0,
