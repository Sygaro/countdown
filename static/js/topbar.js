...
export const Topbar = (() => {
  ...
  function fmtMMSS(ms) { const neg = ms < 0, abs = Math.abs(ms); const s = Math.floor(abs/1000), mm = Math.floor(s/60), ss = s%60; return (neg ? "−" : "") + `${mm}:${pad(ss)}`; }
  function setDot(root, selector, tone) { const dot = qs(selector, root); if (dot) dot.className = "dot " + (tone || "warn"); }
...
function ntpStateFrom(about) {
  ...
  if (!t_ms && ntp.LastSyncUSec) {
    const us = Number(ntp.LastSyncUSec); if (isFinite(us) && us > 0) t_ms = Math.floor(us / 1000);
  }
  if (!t_ms && ntp.LastSyncISO) {
    const d = Date.parse(ntp.LastSyncISO); if (!Number.isNaN(d)) t_ms = d;
  }
  ...
  // terskler
  let tone = "ok";
  if (ageHours > 24) tone = "bad";
  else if (ageHours > 6) tone = "warn";
  ...
  async function call(path, body, method = "POST") {
    const res = await fetch(path, { method, headers: { "Content-Type": "application/json", ...authHeaders() }, body: method === "GET" ? null : JSON.stringify(body || {}) });
  ...
  function init(opts) {
    ...
    const updateClock = () => { qs("#tb_clock", root).textContent = fmtClock(new Date()); };
    updateClock(); setInterval(updateClock, 1000);

    async function pollTick() {
      try { const r = await fetch("/tick", { cache: "no-store" }); const t = await r.json(); qs("#tb_cd", root).textContent = fmtMMSS(t.signed_display_ms); } catch {}
    }
    ...
    pollSvcs(); setInterval(pollSvcs, 10000);
    ...
    function bind(btn, fn) {
      ...
      b.addEventListener("click", async () => {
        b.disabled = true;
        try { await fn(); } catch (e) { console.error(e); alert("Feil: " + e.message); }
    ...
    bind("#tb_shutdown", async () => {
      ...
      if (!typed || typed.trim().toLowerCase() !== "ja") { alert("Shutdown avbrutt."); return; }
      await call("/api/sys/shutdown", {});
    });

    bind("#tb_selftest", async () => {
      if (typeof window.__runSelftest === "function") { await window.__runSelftest(); document.getElementById("selftest")?.scrollIntoView({ behavior: "smooth", block: "center" }); }
      else { const r = await fetch("/debug/selftest", { cache: "no-store" }); const js = await r.json(); alert(js?.ok ? "Selvtest: OK" : "Selvtest: feil"); }
    });
...
try { window.Topbar = Topbar; } catch {}
