// static/js/diag_widget.js
// Flytende widget for live tick-diagnostikk. Viser state, phase, RTT m.m.
  ...
  function buildPanel() {
    ...
    async function once() {
      try {
        ...
        document.getElementById("dw-phase").textContent = j.phase || "—";
        ...
        // Display med minus-prefiks i overrun
        let sign = "";
        if (typeof j.signed_display_ms === "number" && j.signed_display_ms < 0) {
          sign = "-";
        } else if (j.state === "overrun" || j.phase === "over") {
          sign = "-";
        }
        ...
        status.textContent = "Feil: " + (e?.message || String(e));
      }
    }
