...
(function () {
  ...
  `;
  function injectStyle(){ if(!document.getElementById('dw-style')){ const s=document.createElement('style'); s.id='dw-style'; s.textContent=STYLE; document.head.appendChild(s);} }
  function el(t,c,txt){ const e=document.createElement(t); if(c) e.className=c; if(txt!=null) e.textContent=txt; return e; }
  async function fetchTick(){ const t0=performance.now(); const r=await fetch('/tick',{cache:'no-store'}); const t1=performance.now(); return { j: await r.json(), rtt: Math.round(t1-t0) }; }
  function mmss(ms){ const s=Math.max(0, Math.floor(Number(ms||0)/1000)); const mm=Math.floor(s/60); const ss=s%60; return (mm<10?'0'+mm:mm)+':'+(ss<10?'0'+ss:ss); }

  function buildPanel(){
    const wrap=el('div','dw-wrap'), card=el('div','dw-card'), head=el('div','dw-h'), ttl=el('div','ttl','Live Tick Diagnostics'), ctrls=el('div','ctrls');
    const btn=el('button','dw-btn','Pause'), ivLbl=el('label','','Intervall (ms): '), iv=el('input'), status=el('span','dw-mono','—');
    iv.type='number'; iv.min='200'; iv.step='100'; iv.value='1000';
    ctrls.append(btn, ivLbl, iv, status); head.append(ttl, ctrls);

    const body=el('div','dw-body'), grid=el('div','dw-grid dw-mono'), raw=el('div','dw-json dw-mono');
    const rows=[['State','state'],['Mode','mode'],['Blink','blink'],['Server now_ms','now'],['Client now_ms','cnow'],['Δ (client-server)','delta'],['Target ms','tms'],['Target HH:MM','thhmm'],['Display mm:ss','disp'],['warn_ms','warn'],['alert_ms','alert'],['overrun_ms','overrun'],['Config flags','flags'],['Config path','cfgp']];
    rows.forEach(([k,key])=>{ const a=el('div','',k),b=el('div','', '—'); b.id='dw-'+key; grid.append(a,b); });

    body.append(grid, raw); card.append(head, body); wrap.append(card); document.body.appendChild(wrap);
    ...
    async function once(){
      try{
        ...
        const clientNow=Date.now(), serverNow=Number(j.now_ms||0), delta=clientNow - serverNow;
      ...
      }catch(e){ status.textContent='Feil: '+(e&&e.message?e.message:String(e)); }
    }
    function loop(){ stop(); const n=Math.max(200, Math.floor(Number(iv.value||1000))); timer=setInterval(()=>{ if(!paused) once(); }, n); }
    function stop(){ if(timer){ clearInterval(timer); timer=null; } }
    btn.addEventListener('click', ()=>{ paused=!paused; btn.textContent=paused?'Fortsett':'Pause'; if(!paused) once(); });
    iv.addEventListener('change', loop);

    once(); loop(); window.addEventListener('beforeunload', stop);
  }

  function init(){ injectStyle(); if(!document.querySelector('.dw-wrap')) buildPanel(); }
