...
export const ui = (function(){
  ...
  function toast(msg, tone){
    ...
    el.style.border = '1px solid ' + (tone==='bad' ? 'rgba(255,80,80,.6)' : 'rgba(255,255,255,.4)');
    el.style.opacity = '1';
  ...
  function get(url){ return fetch(url, { cache:'no-store' }).then(_handle); }

  function post(url, body, opts){
    opts = opts || {};
    var headers = {'Content-Type': 'application/json'};
    if (opts.password) headers['X-Admin-Password'] = opts.password;
    return fetch(url, { method:'POST', headers: headers, body: JSON.stringify(body || {}) }).then(_handle);
  }
  ...
  return { qs:qs, qsa:qsa, toast:toast, get:get, post:post, activateNav:activateNav, mmss:mmss, signedMmss:signedMmss };
})();

try { window.ui = window.ui || ui; } catch(e) {}
