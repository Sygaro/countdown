...
function toast(msg, tone) {
  ...
  el.style.border = "1px solid " + (tone === "bad" || tone === "error" ? "rgba(255,80,80,.6)" : "rgba(255,255,255,.4)");
  el.style.opacity = "1";
  ...
  async function call(path, payload, method = "POST") {
    const res = await fetch(path, { method, headers: authHeaders(), body: method === "GET" ? null : JSON.stringify(payload || {}) });
  ...
  // ---------- NTP-detaljer ----------
  function getNtpTarget() { return qs("#ntp_details") || qs("#ntp_status"); }
  function writeNtp(target, linesOrHtml) {
    ...
    if (isPre) target.textContent = Array.isArray(linesOrHtml) ? linesOrHtml.join("\n") : String(linesOrHtml || "");
      ...
      if (Array.isArray(linesOrHtml)) {
        target.innerHTML = linesOrHtml.map((l) => {
...
function bindSysButtons() {
  ...
  async function pollLive() {
    ...
    qs("#meta")      && (qs("#meta").textContent = `mål: ${t.target_hhmm || t.target_ms} · nå: ${new Date(t.now_ms).toLocaleTimeString()}`);
  ...
  function init() {
    ...
    document.querySelectorAll("[data-dump]").forEach((btn) => btn.addEventListener("click", () => dump(btn.dataset.dump).catch(console.error)));
