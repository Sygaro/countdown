...
async function once(){
  ...
  const signed = (typeof j.signed_display_ms === 'number') ? j.signed_display_ms
                 : (j.state === 'overrun' || j.mode === 'over') ? -Number(j.display_ms||0)
...
function loop(){
  ...
  timer = setInterval(()=>{ if(!paused) once().catch(()=>{}); }, iv);
}
function stop(){ if(timer){ clearInterval(timer); timer=null; } }

function init(){
  ui.activateNav('/diag');
  ui.qs('#toggle').addEventListener('click', ()=>{ paused=!paused; ui.qs('#toggle').textContent = paused ? 'Fortsett' : 'Pause'; if(!paused) once(); });
